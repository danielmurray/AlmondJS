...
static csmi_sb_handle_t read_sb( csmi_uboot_cmds_struct_t *s,
				 unsigned long addr );

csmi_uboot_cmds_handle_t csmi_uboot_cmds_initialize
...
int csmi_uboot_cmds_sb_display
...
int csmi_uboot_cmds_sb_set_flags
( csmi_uboot_cmds_handle_t handle,
  unsigned long sb_address,
...
int csmi_uboot_cmds_sb_process
( csmi_uboot_cmds_handle_t handle,
  int argc, char *argv[] )
{
  csmi_uboot_cmds_struct_t *s = (csmi_uboot_cmds_struct_t *) handle;
  ...
  for( i=0; i<SB_SUPPORT; i++ ) {
    sbinfo[i].address = 0; sbinfo[i].s_address = NULL; sbinfo[i].sb = NULL;
  ...
  for( i=(SB_SUPPORT-1); i>=0; i-- ) {
	...
	if ( kernel[0] == '0' && kernel[1] == 'x' ) {
	  ...
	  if ( s->cp_jffs2_ram( kernel, kaddr ) != 
	       csmi_sb_file_size( h ) ) {
	    // Copy failed
	...
	char *rootfs = csmi_sb_rootfs( sbinfo[i].sb );
	  ...
	  if ( rootfs[0] == '0' && rootfs[1] == 'x' ) {
	    ...
	    if ( s->cp_jffs2_ram( rootfs, raddr ) != 
		 csmi_sb_file_size( h ) ) {
	      // Copy failed
	      printf( "Failed to copy %s to %lx\n", rootfs, raddr );
	      csmi_sb_set_valid( sbinfo[i].sb, 0 );
...
DONE:
	...
	printf( "Skipping SB at %s because v=%d, c=%d, a=%d\n",
		sbinfo[i].s_address,
                csmi_sb_valid(  sbinfo[i].sb ),
                csmi_sb_commit( sbinfo[i].sb ),
                csmi_sb_active( sbinfo[i].sb ) );
      }
    }
    else {
      // sb[i] == NULL
...
csmi_sb_handle_t read_sb( csmi_uboot_cmds_struct_t *s,
			  unsigned long addr )
{
