...
unsigned int csmi_sb_mgmt_boot( csmi_sb_handle_t handle )
{
  csmi_sb_s *o = (csmi_sb_s *) handle;
  if ( o == NULL ) return( 0 );
  return( o->flags.bit.mgmt_boot );
}
...
void csmi_sb_set_mgmt_boot( csmi_sb_handle_t handle, unsigned char yesno )
{
  csmi_sb_s *o = (csmi_sb_s *) handle;
  if ( o == NULL ) return;
  o->flags.bit.mgmt_boot = yesno;
  redo_md5( o );
}
